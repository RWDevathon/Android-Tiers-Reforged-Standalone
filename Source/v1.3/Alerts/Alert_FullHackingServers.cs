...
namespace ATReforged
{
    public class Alert_FullHackingServers : Alert
    {
        ...
        {
            if (!ATReforged_Settings.playerCanHack || !ATReforged_Settings.receiveHackingAlert || Utils.gameComp.GetHackingPointCapacity() <= 0)
                return false;
            ...
            float points = Utils.gameComp.GetHackingPoints();
            if (points >= Utils.gameComp.GetHackingPointCapacity() * 0.9f && points >= Utils.gameComp.hackCostTimePenalty + 400)
            {
                return true;
            }
            return false;
